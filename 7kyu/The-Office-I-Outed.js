...
function outed(meet, boss){
    //   calculate all values of keys together
    // get boss value , add to overall and return

      let values = Object.values(meet)
      const numOfPeople = Object.keys(meet).length
      let meetup = values.reduce((acc, cval) => {
        return acc + cval
      })
     
    //   Getting boss value from object
        const bossvalue = meet[boss];
        meetup = meetup + bossvalue
        //calculate happiness rate
      let happinessRate = meetup/numOfPeople
